        ...
        >
          <thead>
            <tr>
              ...
              <th v-if="actionsOne.length">Detalles</th>
              <th v-if="actionsTwo.length">Acciones</th>
            </tr>
          </thead>
            ...
            >
              ...
              >
                ...
                <span
                  ...
                  :class="td.class"
                  v-html="
                    td.rendered != null ? formatValue(td.rendered) : '----'
                  "
                >
                </span>
                <span
              ...
              <!-- Diplay Actions If Provided -->
              <td v-if="item.buttonsTwo.length">
                <!-- Loop Through All Provided Actions -->
                <button
                  type="button"
                  class="btn badge rounded-pill"
                  :class="`btn-${button.color} btn-${button.size}`"
                  v-bind:key="j"
                  v-for="(button, j) in item.buttonsTwo"
                  @click="button.action(item.row, item.index)"
                  v-if="button.show"
                  :disabled="button.disabled"
                >
          ...
          <tbody v-else>
            ...
            <tr>
              <td
                ...
                :colspan="
                  headers.length +
                  (actionsOne.length ? 1 : 0) +
                  (actionsTwo.length ? 1 : 0) +
                  (index ? 1 : 0)
                "
              >
                No results
        ...
        <div class="col-md-6" v-if="paginate">
          <ul class="pagination" v-if="paginateLinks.length">
            ...
            <li class="page-item" v-if="pages && currentPage < pages">
              <span class="page-link" @click="next">Siguiente</span>
            </li>
          </ul>
        </div>
      </div>
    ...
    formatValue(value) {
      ...
      temp_value =
        regex.test(value)
          ...
          : "Invalid Date";
      //console.log(temp_value == "Invalid Date")
      if (temp_value == "Invalid Date") {
        new_value = value;
      } else {
        new_value = temp_value.toLocaleString("es-GT").split(" ")[0];
      }
      return new_value;
    },
    mapItems(items) {
      items = items.map((item, index) => {
        ...
        // Get Provided ActionsOne
        this.actionsOne.forEach((button, index3) => {
          row.buttons.push({
        ...
        // Get Provided ActionsTwo
        this.actionsTwo.forEach((button, index3) => {
          row.buttonsTwo.push({
            // Spread Provided Button Properties
            ...button,
            // Decide Visibility Depending On Whether Show Method Is Provided
